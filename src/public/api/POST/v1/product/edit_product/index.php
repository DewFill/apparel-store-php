...
$request->checkRequestVariablesOrError("product_id", "product_name", "product_price");
...
if ($_FILES['product_main_image']['error'] !== UPLOAD_ERR_OK) {
    JsonOutput::error("Upload failed with error code " . $_FILES['product_main_image']['error']);
}
...
$product = ProductsQuery::create()->findOneById($request->getData("product_id"));
